...
//softwar timer handler for xTimerCreate API
...
//This is the menu, string sent over uart
char menu[]={"\
\r\nLED_ON             ----> 1 \
\r\nLED_OFF            ----> 2 \
\r\nLED_TOGGLE         ----> 3 \
...
int main(void)
{
	...
	command_queue = xQueueCreate(10,sizeof(APP_CMD_t*)); //32bit MCU, so 10*4=40 bytes.
														// if not pointer 10*11=110 bytes
	//lets create the write queue
	uart_write_queue = xQueueCreate(10,sizeof(char*));//40bytes`
...
char *pData = menu; // starting address of the char array
...
static void prvSetupUart(void)
{
	...
	//lets enable the UART byte reception interrupt in the mcu
...
void USART2_IRQHandler(void)
{
	...
	if( USART_GetFlagStatus(USART2,USART_FLAG_RXNE) )
	{
		...
		command_buffer[command_len++] = (data_byte & 0xFF) ; // want last 8 bits, mask others

		if(data_byte == '\r') //if Enter key is pressed (endoftyping)
		{
...
void led_toggle(TimerHandle_t xTimer) // TimerHandle_t for xTimerCreate API
{
...
void led_toggle_start(uint32_t duration)
{
	...
	if(led_timer_handle == NULL)
	{
		...
		xTimerStart(led_timer_handle,portMAX_DELAY); // xTimerStart actually puts msg on TimerQueue
	}
	else
	{
